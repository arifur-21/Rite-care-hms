...
Future<void> _getRefresh () async{
 await otListVM.getSchedule();
                      ...
                      if(otListVM.otScheduleList.value.items?.length == 0 || otListVM.otScheduleList.value.items?.length == "" || otListVM.otScheduleList.value.items?.length == null){
                        ...
                        return ListView.builder(
                            ...
                            itemBuilder: (context, index){
                              print("ot status ${otListVM.otScheduleList.value.items![index].item?.name}");
                              Text("sasdf ${otListVM.otScheduleList.value.items![index].item?.name}");
                              return  otListWidget(
  ...
  Widget otListWidget({ dynamic statusId, dynamic? title, dynamic? name, dynamic? status, dynamic? surgeryType,required int indexNum, dynamic noteId, required List<Items> suergeryList, required OtScheduleModel otScheduleModel}){
///suergeryList[0].patient?.firstName
   //print("object ot ${otScheduleModel.items![indexNum].surgeryStatus?.name}");
  // dynamic data = otScheduleModel.items![indexNum].surgeryStatus;
