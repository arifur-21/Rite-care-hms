...
class _OtManagementScreenState extends State<OtManagementScreen> {
                    ...
                    child: Obx (() {
                      ...
                      switch(otListVM.rxRequestStatus.value) {
                        ...
                        case Status.ERROR:
                          ...
                          print("error 11 ${otListVM.error.value.toString()}");
                            ...
                            return ListView.builder(
                                ...
                                itemBuilder: (context, index){
                                  return otListWidget(
                                      ...
                                      suergeryList: [otListVM.otScheduleList.value.items![index]],
                                      otScheduleModel: otListVM.otScheduleList.value,
                                      surgeryNotes:  otListVM.otScheduleList.value.items?[index].surgeryNotes
                                  );
                                });
  ...
  Widget otListWidget({ dynamic statusId, dynamic? title, dynamic? name, dynamic? status, dynamic? surgeryType,required int indexNum, dynamic noteId, required List<Items> suergeryList, required OtScheduleModel otScheduleModel, List<SurgeryNotes>? surgeryNotes }){
                  ...
                  children: [
                    ...
                    InkWell(
                      onTap: (){
                        ...
                        _showDialog(status: status, title: title, name: name, surgeryType: surgeryType, indexNum: indexNum, noteId: noteId, suergeryList: [suergeryList[0]], otScheduleModel: otScheduleModel );
                      },
                      child: Visibility(
                        ...
                        child: Container(
                            ...
                            child: Center(
                                child: Text("${(status == 'Waiting')? startBtn : (status == 'Started') ? endBtn : (status == 'Completed') ? btnVisibility : startBtn }",  style: TextStyle(color:Colors.white, fontSize: 20, fontWeight: FontWeight.bold)))),
                      ),
                    ),
                    ...
                    InkWell(
                        onTap: (){
                          Navigator.push(context, MaterialPageRoute(builder: (context)=>OtManagementDetailsScreen(
                            noteId: noteId,
                            id: otScheduleModel.items![indexNum].id,
                            userId: otScheduleModel.items![indexNum].userId,
                          )));
                        },
                        child: Icon(Icons.file_copy_outlined, size: 40,))
                  ],
                )
              ],
            ),
...
//
  ...
  _showDialog  ({dynamic? title, dynamic? name, dynamic? status, dynamic? surgeryType,int? indexNum, dynamic noteId, required List<Items> suergeryList, required OtScheduleModel otScheduleModel}) async {
    await showDialog(
        context: context,
        builder: (context) {
                ...
                InkWell (
                    onTap: () {
                      ...
                      otListVM.operationScheduleStatus(
                        statusId:statusId,
                        noteId: noteId,
                        userId: suergeryList[0].userId,
                        surgeryTypeId: suergeryList[0].surgeryTypeId,
                        roomId: suergeryList[0].roomId,
                        surgeryType_id1: suergeryList[0].surgeryType?.id,
                        surgeryType_name1: suergeryList[0].surgeryType?.name,
                        disease: suergeryList[0].disease,
                        ans: suergeryList[0].aNS,
                        asa: suergeryList[0].aSA,
                        ward: suergeryList[0].ward,
