...
class _RegistrShortFormState extends State<RegistrShortForm> {
  ...
  Future<List<NumberCheckModel>> getPhoneNumber({dynamic number})async {
    var data;
    ...
    final response = await http.get(Uri.parse('https://mobileapp.rite-hms.com/Patient/GetPatientByPhone?phoneNumber=${number}'),
        headers: {
    ...
    if(response.statusCode == 200){
      ...
      for(Map i in data){
        NumberCheckModel numberCheckModel = NumberCheckModel(
          phoneNumber: i['PhoneNumber']
        );
        numberCheckList.add(numberCheckModel);
      }
