...
class SummeryDetailsViewModel {
  ...
  void setLabTestList(LabTestListModel _value) =>
      labtestListData.value = _value;
  ...
  void setLabTestNameSugg(List<LabTestNameSuggModel> _value) =>
      labTestNameSuggList.value = _value;
  ...
  void getSampleListData(
      {dynamic statusId = 0, dynamic page = 1, dynamic serviceId = 0}) async {
    setRxRequestStatus(Status.LOADING);

    await loginPreference
        .getServiceId()
        .then((value) => {serviceId = value.setviceId});
    print("service sample  $serviceId");
    await _api
        .getSampleListData(startDate, endDate, statusId, page, serviceId)
        .then((value) {
      setRxRequestStatus(Status.SUCCESS);
      setSampleList(value);
    }).onError((error, stackTrace) {
  ...
  void getSummeryListData(
      {statusId = 0, dynamic page = 1, dynamic serviceId = 0}) async {
    print("test");
    setRxRequestStatus(Status.LOADING);
    ...
    await _api
        .getSummeryListData(startDate, endDate, statusId, page, serviceId)
        .then((value) {
      setRxRequestStatus(Status.SUCCESS);
  ...
  Future<void> saveEditLabReportResult({dynamic result}) async {
  ...
  /// operation Schedule status
  void updatePatientServiceMarkAsPrintStatus(
      dynamic statusId, dynamic status) async {
