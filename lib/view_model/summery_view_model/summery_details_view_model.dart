...
class SummeryDetailsViewModel extends GetxController{
  ...
  /// operation Schedule status
  Future<void> updateEditLabReportResult({dynamic result, dynamic id}) async {
    print("text controller ${textController.value.text}");
    print("result : $result");
    await _repository.updateEditLabReportResult(result).then((value) {
      print("design respons vm----------------> $value");
      Utils.snakBar("status", 'status update successfull');
    }).onError((error, stackTrace) {
      Utils.snakBar("Error", error.toString());
      print("error occured : ${error.toString()}");
