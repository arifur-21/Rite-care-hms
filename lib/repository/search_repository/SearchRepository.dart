...
class SearchRepository{
  ...
  Future<SampleTest> getSampleListData(startDate, endDate,statusId, page, serviceId)async{
    dynamic response = await _apiServices.getApiData('/Item/GetInvoiceSampleIDByMedicalType?id=0&statusid=${statusId}&medicalTypeID=62&DateStart=${startDate}&DateEnd=${endDate}&pageNumber=1&pageSize=25&invoiceId=undefined&sampleId=null');
  ...
  Future<LabReportResultModel> getTableRowDesignItem(serviceId, itemId)async{
    print("repo item Id ${itemId}");
    print("repo service Id ${serviceId}");
    dynamic response = await _apiServices.getApiData('/Item/GetTableRowDesignByItemId?itemId=$itemId&machineId=&groupItemId=&serviceId=933702');
    return LabReportResultModel.fromJson(response);
  }
