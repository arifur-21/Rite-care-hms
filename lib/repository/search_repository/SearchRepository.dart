...
class SearchRepository{
  ...
  Future<SampleTest> getSampleListData(startDate, endDate,statusId, page, serviceId)async{
    dynamic response = await _apiServices.getApiData('/Item/GetInvoiceSampleIDByMedicalType?id=775925&statusid=0&medicalTypeID=62&DateStart=$startDate&DateEnd=$endDate&pageNumber=1&pageSize=25&invoiceId=undefined&sampleId=null');
    return SampleTest.fromJson(response);
  }

/*  //get summery list data
  Future<SummeryModel> getSummeryListData(statusId, page, serviceId,startDate, endDate, invoiceId)async{
    print("repo service id $serviceId");
    dynamic response = await _apiServices.getApiData('/Item/GetPatientInvoicebyMedicalType?id=$serviceId&statusid=$statusId&medicalTypeID=62&DateStart=${startDate}&DateEnd=${endDate}&pageNumber=$page&pageSize=25&invoiceId=$invoiceId&sampleId=null&itemId=undefined');
  ...
  Future<SummeryModel> getSummeryListData(startDate, endDate,statusId, page, serviceId)async{
    dynamic response = await _apiServices.getApiData('/Item/GetPatientInvoicebyMedicalType?id=${serviceId}&statusid=$statusId&medicalTypeID=62&DateStart=${startDate}&DateEnd=${endDate}&pageNumber=1&pageSize=25&invoiceId=undefined&sampleId=null&itemId=undefined');
    return SummeryModel.fromJson(response);
  }
  ...
  Future<TableRowDesignModel> getTableRowDesignItem(serviceId, itemId)async{
    ...
    print("repo service Id ${serviceId}");
    dynamic response = await _apiServices.getApiData('/Item/GetTableRowDesignByItemId?itemId=$itemId&machineId=&groupItemId=&serviceId=');
