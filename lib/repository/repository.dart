...
class Repository{
  ...
  Future<dynamic> saveEditLabReportResult(data)async{
    print("repo report result ${jsonEncode(data)}");
    dynamic response = await _apiServices.postApiData(data, '/Item/SaveEditLabReportResult');
    return response;
  }
  ...
  Future<LabTestListModel> getLabTestListApi(int labStatus, int page)async{
    print("page reo ${page}");
    print("labtest status id reo  ${labStatus}");
    final response = await _apiServices.getApiData('/Item/GetLabItemsByMedicalType?medicalTypeID=62&pageNumber=1&pageSize=25&categoryId=${labStatus}&searchTerm=&allData=undefined&ItemId=0&isLabItemSerialNo=false');
