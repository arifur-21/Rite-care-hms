...
class Repository{
  ...
  Future<LabTestListModel> getLabTestListApi(dynamic labStatus, dynamic page)async{
    ...
    dynamic response = await _apiServices.getApiData('${AppUrl.base_url}/Item/GetLabItemsByMedicalType?medicalTypeID=62&pageNumber=${page}&pageSize=25&categoryId=${labStatus}&searchTerm=&allData=undefined&ItemId=0&isLabItemSerialNo=false');
  ...
  Future<List<RankModel>> getUnitListData()async{
    dynamic response = await _apiServices.getListOfApiData('${AppUrl.base_url}/Patient/GetAllUnitList?search=');
    unitList.clear();
    for(Map i in response){
      RankModel data =  RankModel.fromJson(i);
      unitList.add(data);
